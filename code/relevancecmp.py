...
def compare_relevance(reference, R0, R1, shape=None):
    difference = R0-R1
    ...
    print(f"Normalized\n average absolute relevance difference: {naard*10:.2f}%")
    ...
    b = 10*(np.abs(reference_rs)**3.0).mean()**(1.0/3)
    axs[0][0].imshow(reference_rs, cmap="gray")
    axs[0][0].set_title("Reference image")

    b = 10*(np.abs(R0_rs)**3.0).mean()**(1.0/3)
    axs[0][1].imshow(R0_rs, cmap=my_cmap)
    axs[0][1].set_title("Relevance 0")

    b = 10*(np.abs(R1_rs)**3.0).mean()**(1.0/3)
    axs[0][2].imshow(R1_rs, cmap=my_cmap)
    axs[0][2].set_title("Relevance 1")
